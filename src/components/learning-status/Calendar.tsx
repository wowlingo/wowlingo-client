...
const Calendar = ({ selectedDate, onDateSelect, onMonthChange, loginedDates, attemptedDates }: CalendarProps) => {
    ...
    return (
        <div className="flex flex-col">
            ...
            {/* 요일 표시 */}
            <div className="grid grid-cols-7 gap-y-2 text-center text-sm mb-3">
            ...
            <div className="grid grid-cols-7 grid-rows-6 flex-grow">
                {calendarGrid.map((date, index) => {
                    ...
                    if (isToday && !isSelected) dateColorClass = 'text-white bg-gray-400 rounded-full font-bold';
                    if (isSelected) dateColorClass = 'text-blue-600 font-bold';

                    return (
                        <div
                            key={date}
                            className={`flex flex-col items-center justify-start h-18 cursor-pointer pt-1 pb-1 rounded-lg transition-colors ${isSelected ? 'bg-blue-100 border border-blue-300' : 'border border-transparent'}`}
                            onClick={() => onDateSelect(fullDate)}
                        >
                            <span className={`text-sm mb-1 w-6 h-6 flex items-center justify-center ${dateColorClass}`}>
                                {date}
                            </span>
